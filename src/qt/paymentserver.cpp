...
const QString BITCOIN_IPC_PREFIX("ekwicoin:");
const char* BITCOIN_REQUEST_MIMETYPE = "application/ekwicoin-paymentrequest";
const char* BITCOIN_PAYMENTACK_MIMETYPE = "application/ekwicoin-paymentack";
const char* BITCOIN_PAYMENTACK_CONTENTTYPE = "application/ekwicoin-payment";
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ekwicoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseEkwicoinURI(arg, &r) && !r.address.isEmpty())
            {
                CEkwicoinAddress address(r.address.toStdString());
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click ekwicoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start ekwicoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling ekwicoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on ekwicoin: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in ekwicoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ekwicoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseEkwicoinURI(s, &recipient))
            {
                CEkwicoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Ekwicoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CEkwicoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Insecure payments to custom ekwicoin addresses are not supported
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            emit message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(EkwicoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
